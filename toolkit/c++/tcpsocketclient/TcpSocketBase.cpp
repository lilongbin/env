...
TcpSocketBase::TcpSocketBase()
{
    ...
    m_localAddr.sin_addr.s_addr = inet_addr(TCP_RECEIVER_IP);
    m_localAddr.sin_port = htons(TCP_RECEIVER_PORT);
    /* set server addr for connect */
    bzero(&m_servAddr, sizeof(m_servAddr));
    m_servAddr.sin_family = AF_INET;
    m_servAddr.sin_addr.s_addr = inet_addr(TCP_SENDER_IP);
    m_servAddr.sin_port = htons(TCP_SENDER_PORT);
}
...
bool TcpSocketBase::socketCreate()
{
    bool ret = false;
    int socketFd = -1;
...
bool TcpSocketBase::socketConnectNonBlock()
{
...
int32_t TcpSocketBase::socketReadData(char * buffer, const int32_t &size)
{
    int16_t readRet = 0;
    int16_t readLen = 0;
    int ready = 0;
    ...
    do {
        ...
        tval.tv_sec = (TCP_SOCKET_READ_TIMEOUT_MILLIS / 1000); /* seconds */
        tval.tv_usec = (TCP_SOCKET_READ_TIMEOUT_MILLIS % 1000);
        ...
        if (ready < 0) {
            ...
            if (select_retry_count >= TCP_SOCKET_READ_TIMEOUT_MAX_COUNT) {
...
int32_t TcpSocketBase::socketWriteData(char * buffer, const int32_t &size)
{
    int32_t offset = 0;
    int32_t left = size;
    int32_t ret = 0;
...
bool TcpSocketBase::isSocketConnected()
{
    return m_socketFd >= 0;
}

bool TcpSocketBase::getSocketFd(int32_t & socketFd)
{
    if (m_socketFd < 0) {
        return false;
    }
...
bool TcpSocketBase::socketConfigure()
{
    bool ret = false;
...
bool TcpSocketBase::setNonBlocking(bool nonblocking)
{
    int opts;
...
void TcpSocketBase::socketClose()
{
    if (m_socketFd > 0) {
        #if 1
