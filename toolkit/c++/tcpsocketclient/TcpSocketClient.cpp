...
/*-----------TcpSocketClient--begin-----------*/
TcpSocketClient::TcpSocketClient()
{
    m_socketClient = new TcpSocketBase();
...
bool TcpSocketClient::initSocketClient()
{
    bool retValue = false;
    int times = 0;
...
bool TcpSocketClient::isTcpConnected()
{
    bool ret = false;
...
bool TcpSocketClient::doConnectToServer()
{
    if (NULL == m_socketClient) {
        ALOGW("%s m_socketClient is NULL", __func__);
        return false;
...
void TcpSocketClient::disConnectToServer()
{
    if ((m_socketClient != NULL)
...
bool TcpSocketClient::doWriteData(uint8_t * buf, const uint32_t &maxSize, uint32_t &len)
{
    int32_t retValue;
    len = 0;
...
bool TcpSocketClient::doReadData(uint8_t * buf, const uint32_t &maxSize, uint32_t &len)
{
    return doReadDataBlock(buf, maxSize, len);
}

bool TcpSocketClient::doReadDataBlock(uint8_t * buf, const uint32_t &maxSize, uint32_t &len)
{
...
bool TcpSocketClient::doReadDataTimeout(uint8_t * buf, const uint32_t &maxSize, uint32_t &len)
{
    int32_t ret = 0;
    int socketFd = 0;
    int32_t retValue = -1;
