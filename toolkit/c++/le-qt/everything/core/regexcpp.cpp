...
string research(string re, string src, int cflags=REG_EXTENDED)
{
   // 只要找到第一个就返回，否则返回空
   string result;
   regmatch_t match;
...
void test()
{
   ...
   cout << "match: " << rematch(pattern, str) << endl;
   result = refindall(pattern, str);
   cout << "findall: ";
   ...
   cout << "search: " << research(pattern, str) << endl;
   cout << "raw str: " << str << endl;
   cout << "sub str: " << resub(pattern, "***", str) << endl;
   cout << "sub str: " << resub(pattern, "###", str, 2) << endl;
   cout << "sub str: " << resub("#$%&@", "###", str, 2) << endl;
}
