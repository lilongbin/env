...
2. then change current directory to the source code location.
 ...
 then use below commands to generate cscope files and tags in order to exploring source code;
	$ /bin/bash ~/toolkit/shell/gen_kernel_SrcExpl.sh      #for kernel
	$ /bin/bash ~/toolkit/shell/gen_android_SrcExpl.sh     #for android
...
then use 'dp' (diff put) or 'do' (diff get)
...
open vim diff:
	$ vim -d file1 file2
put diff patch to the other file:
	:dp
get diff patch from the other file:
	:do
...
enter normal mode, press qa(or qb/qc, etc.); start to record your operation and save to a/b/c register;
backto normal mode, press q to stop record;
press @a or N@a (N is a number)to play your record;

7.5 write file use root
